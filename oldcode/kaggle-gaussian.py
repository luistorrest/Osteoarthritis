...
#@staticmethod
def read_split_data(root, plot_image=False,save_path='./'):
    filepaths = []
    labels = []
    bad_images = []
    ...
    with open(os.path.join(save_path,'classes_indices.json'), 'w') as json_file:
    ...
    if plot_image:
        ...
        #plt.show()
        plt.savefig(os.path.join(save_path,"kaggle_class-distribution.png"))
        plt.close("all")
...
class NeuralNetwork():
    ...
    def __init__(self, root='tmp/', epochs=5, batch_size=1, learning_rate=0.0003, model_name=None, plot_roc=True,
                 predictor=True, plot_acc_loss=True, best_val_acc=0, save_path='./Weights', use_multi_gpus=False,
                 plot_image_class_distribution=True, save_name='./Weights/model.pth', csv_path='./ValAccuracy.csv',samples_likelihood=100):
        ...
        self.samples_likelihood = samples_likelihood

        self.data_transform = TRANSFORMS

    def train_step(self, net, optimizer, data_loader, device, epoch):
        net.train()
        loss_function = gpytorch.mlls.VariationalELBO(net.likelihood, net.classifier.gp_layer, num_data=len(data_loader.dataset))
        #nn.CrossEntropyLoss()
        train_acc, train_loss, sampleNum = 0, 0, 0
        optimizer.zero_grad()

        with gpytorch.settings.num_likelihood_samples(self.samples_likelihood):
            ...
            for step, data in enumerate(train_bar):
                ...
                optimizer.zero_grad()
                outputs = net(images)
                loss = loss_function(outputs, labels)*-1 # -1 only for mll of the gaussian part

                # gaussian
                pred = outputs.probs.mean(0).argmax(-1)  # Taking the mean over all of the sample we've drawn
                train_acc += pred.eq(labels.view_as(pred)).cpu().sum()
    ...
    def fit(self):
        train_image_path, train_image_label, val_image_path, val_image_label, class_indices = read_split_data(
            self.root, self.plot_image_class_distribution,self.save_path)
        ...
        class DKLModel(gpytorch.Module):
            ...
            def forward(self, x):
                ...
                # This next line makes it so that we learn a GP for each feature
                features = features.transpose(-1, -2).unsqueeze(-1)
                #features = torch.squeeze(torch.flatten(features))
                res = self.gp_layer(features)
                return res
        ...
        class ResNet(nn.Module):
            def __init__(self, num_classes) -> None:
                ...
                self.classifier = DKLModel(num_dim=self.features.fc.out_features)
                self.likelihood = gpytorch.likelihoods.SoftmaxLikelihood(num_features=self.features.fc.out_features, num_classes=num_classes)
                #ResMLP(initial_units=1000, block_list=[2, 2, 2, 2], num_classes=num_classes)
    ...
    def Plot_ROC(self, net, val_loader):
        ...
        # plt.savefig(os.path.join(self.save_path,'set113_roc.jpg')
        #plt.show()
        plt.savefig(os.path.join(self.save_path,"kaggle_roc.png"))
        plt.close("all")
    ...
    def Plot_Acc_Loss(self, df):
        ...
        plt.tight_layout()
        #plt.show()
        plt.savefig(os.path.join(self.save_path,"kaggle_accuracy.png"))
        plt.close("all")
    ...
    def Predictor(self, net, test_loader):
        ...
        #plt.show()
        plt.savefig(os.path.join(self.save_path,"kaggle_confusion-matrix.png"))
        plt.close("all")
...
if __name__ == '__main__':
    root_path=r"C:\Users\grupo_gepar\Documents\lucho\Osteo\Osteoarthritis_Assignment_Merged_Prep"
    #r"C:\Users\grupo_gepar\Documents\lucho\Osteo\archivePrep\merged"
    #root_path=r"C:\Users\grupo_gepar\Documents\lucho\Osteo\Osteoarthritis_Assignment_Merged"
    model_root=r"C:\Users\grupo_gepar\Documents\lucho\Osteo\Codigo"
    
    modelfolder="kagglemodel-our-results2"

    model_path=os.path.join(model_root,modelfolder)

    save_name=os.path.join(model_path,"model.pth")
    csv_path=os.path.join(model_path,"val.csv")
    os.makedirs(model_path,exist_ok=True)
    model = NeuralNetwork(root=root_path,save_path=model_path,save_name=save_name,csv_path=csv_path)
    model.fit()