...
class Net(nn.Module):
    def __init__(self):
        ...
        # Cargar el modelo DenseNet121 pre-entrenado
        ...
        self.densenet.classifier = nn.Linear(in_features, 512)
        # Salida para dos clases
        self.fc = nn.Linear(512, 2)

    def forward(self, x):
        # Pasa la entrada a través del modelo DenseNet121
        x = self.densenet(x)
        # Aplicar relu para introducir no linealidad
        x = F.relu(x)
        # Pasa la salida a través de la capa completamente conectada definida
        x = self.fc(x)
